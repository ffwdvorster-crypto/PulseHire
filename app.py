...
def db_init():
    with db_conn() as con:
        ...
        # --- New: settings table for blocked counties
        con.execute("""
        CREATE TABLE IF NOT EXISTS blocked_counties (
            county TEXT PRIMARY KEY
        )""")
        # --- Migrations (safe to run each start)
        try:
        ...
        try:
            ...
            pass
        # Seed initial blocked counties once if empty
        cur = con.cursor()
...
def set_blocked_counties(counties: List[str]):
    ...
    with db_conn() as con:
        ...
        con.executemany("INSERT OR IGNORE INTO blocked_counties(county) VALUES (?)", [(c.strip(),) for c in counties if c.strip()])

def apply_blocked_counties_to_candidates() -> int:
    """Flag candidates whose county is blocked and not protected by override. Return count changed."""
...
def upsert_candidate(row: Dict[str, Any], is_test: int = 0):
    ...
    # Determine if county is blocked
    blocked = [c.lower() for c in get_blocked_counties()]
    ...
    with db_conn() as con:
        ...
        if email:
            ...
            if hit:
                cid, override = hit
                # Only auto-set DNC if not manually overridden
                auto_dnc_cols = ""
                auto_dnc_vals: List[Any] = []
                if is_blocked and (override or 0) == 0:
                    auto_dnc_cols = ", dnc=?, dnc_reason=?"
                    auto_dnc_vals = [1, "outside of hiring area"]
                cur.execute(f"""
                    UPDATE candidates SET
                        ...
                        updated_at=?
                    WHERE id=?
                """, (name, phone, county, row.get("availability"), row.get("source"),
                      row.get("completion_time"), row.get("notes"), row.get("campaign"),
                      int(is_test), *auto_dnc_vals, now, cid))
            ...
            if hit:
                ...
                cur.execute(f"""
                    UPDATE candidates SET
                        ...
                        updated_at=?
                    WHERE id=?
                """, (email, county, row.get("availability"), row.get("source"),
                      row.get("completion_time"), row.get("notes"), row.get("campaign"),
                      int(is_test), *auto_dnc_vals, now, cid))
        ...
        cur.execute("""
            ...
            VALUES (?,?,?,?,?,?,?,?, 'New','', '', ?, ?, ?, ?, ?, ?, 0)
        """, (email, name, phone, county, row.get("availability"), row.get("source"),
              row.get("completion_time"), row.get("notes"), row.get("campaign"),
    ...
    with st.expander("Upload & Ingest", expanded=False):
        ...
        if up_file is not None:
            try:
                ...
                # Mark the whole upload as test data (optional)
    ...
    if not df.empty:
        ...
        with c1:
            ...
            if st.button("Save changes"):
                # If user unticks DNC, clear reason
                fields = {
                    "status": new_status,
