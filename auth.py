# auth.py — login + RBAC (PBKDF2 with per-user salt)
...
def create_user(email: str, name: str, role: str, password: str):
    ...
    with connect() as con:
        cur = con.cursor()
        cur.execute("INSERT INTO users(email,name,role,password_hash,salt,created_at,updated_at) VALUES(?,?,?,?,?,?,?)",
...
def seed_admin_if_empty():
    with connect() as con:
        ...
        if cur.fetchone()["c"] == 0:
            # default admin: admin@pulsehire.local / admin
